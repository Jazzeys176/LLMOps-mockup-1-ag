...
const Tracing = () => {
    ...
    useEffect(() => {
        const fetchTraces = async () => {
            setLoading(true);
            try {
                const data = await apiClient.getTraces(currentPage, ITEMS_PER_PAGE, searchQuery);
                setTraces(data);
            } catch (err) {
    ...
    if (loading && traces.length === 0) {
        return <div className="p-8 text-center text-slate-400">Loading Traces...</div>;
    }
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    {/* Note: Total count is not returned by API yet, so we just show page */}
                    <p className="text-slate-500 text-sm mt-1">Page {currentPage}</p>
                </div>
            ...
            <div className="bg-[#181D25] rounded-xl border border-slate-800 overflow-hidden">
                                        ...
                                        {trace.scores ? (
                                            <div className="flex gap-2">
                                                ...
                                                <span className="px-1.5 py-0.5 rounded bg-emerald-950/30 text-emerald-500 border border-emerald-900/30 text-[10px] font-medium font-mono">
                                                    context: {trace.scores.context_relevance}
                                                </span>
                                                <span className="px-1.5 py-0.5 rounded bg-emerald-950/30 text-emerald-500 border border-emerald-900/30 text-[10px] font-medium font-mono">
                                                    concise: {trace.scores.conciseness}
                                                </span>
                                            </div>
                                        ) : (
                ...
                <div className="flex items-center justify-between p-4 border-t border-slate-800">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button
                            onClick={() => handlePageChange(currentPage + 1)}
                            disabled={traces.length < ITEMS_PER_PAGE}
                            className={clsx(
                                "p-2 rounded-lg border border-slate-800 text-slate-300 transition-colors",
                                traces.length < ITEMS_PER_PAGE
                                    ? "opacity-50 cursor-not-allowed bg-slate-900"
                                    : "hover:bg-[#1C2028] bg-[#181D25]"
                            )}
                        >
